...
def get_best_sequence(data_by_gene, minGeneCoverage, min_depth_coverage):
    sequence = {}
    probable_sequences = {}
...
def get_results(references_results, minGeneCoverage, min_depth_coverage, typeSeparator, references_files,
                references_headers):
    intermediate_results = {}
    for reference, data_by_gene in references_results.items():
        intermediate_results[reference] = get_best_sequence(data_by_gene, minGeneCoverage, min_depth_coverage)
...
def parse_results(references_results, references_files, references_headers, outdir, minGeneCoverage, min_depth_coverage,
                  typeSeparator):
    references_results = split_references_results_by_references(references_results, references_headers)
    seq_type, seq_type_info, probable_results, improbable_results = get_results(references_results, minGeneCoverage,
                                                                                min_depth_coverage, typeSeparator,
